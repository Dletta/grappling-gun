...
var Room = function(attrs, client, faye, usersResource) {
...
Room.prototype.find = function(id, cb) {
  ...
  .then(function(roomData) {
    return new Room(roomData, this.client, this.faye, this.usersResource);
  }.bind(this));
  return cb ? room.nodeify(cb) : room;
};

Room.prototype.findByUri = function(room_uri, cb) {
...
Room.prototype.join = function(room_uri, cb) {
  var room = Q.all([
    this.usersResource.current(),
    this.findByUri(room_uri)
  ])
  .spread(function(user, room) {
    return this.client.post(this.userPath + '/' + user.id + '/' + 'rooms', {body: {id: room.id}})
    .then(function(roomData) {
      return new Room(roomData, this.client, this.faye, this.usersResource);
    }.bind(this));
  }.bind(this));
...
['users', 'chatMessages'].forEach(function(resource) {
